...
const PACKETS: CapturedPacket[] = [
  {
    no: 1, time: '0.000', src: '192.168.1.105', dst: '8.8.8.8', proto: 'DNS', len: 74,
    ...
    details: { layer: 'Domain Name System (Query)', headers: ['Transaction ID: 0x1a2b', 'Flags: 0x0100 (Standard query)', 'Questions: 1 (A example.com)'] }
  },
  {
    no: 2, time: '0.014', src: '8.8.8.8', dst: '192.168.1.105', proto: 'DNS', len: 90,
    ...
    details: { layer: 'Domain Name System (Response)', headers: ['Transaction ID: 0x1a2b', 'Answers: 1 (93.184.216.34)', 'TTL: 300 seconds'] }
  },
  {
    no: 3, time: '0.022', src: '192.168.1.105', dst: '93.184.216.34', proto: 'TCP', len: 66,
    info: '54321 → 80 [SYN] Seq=0 Win=64240 Len=0',
    hex: '4500 0042 d4f2 4000 4006 c37a c0a8 0169 5d80 d822 d431',
    ascii: 'E..B..@.@..z...i]..".1..SYN...',
    details: { layer: 'Transmission Control Protocol (SYN)', headers: ['Source Port: 54321', 'Destination Port: 80', 'Flags: [SYN]', 'Sequence Number: 0'] }
  },
  {
    no: 4, time: '0.045', src: '93.184.216.34', dst: '192.168.1.105', proto: 'TCP', len: 66,
    ...
    details: { layer: 'Transmission Control Protocol (SYN-ACK)', headers: ['Source Port: 80', 'Destination Port: 54321', 'Flags: [SYN, ACK]', 'Ack Number: 1'] }
  },
  {
    no: 5, time: '0.046', src: '192.168.1.105', dst: '93.184.216.34', proto: 'TCP', len: 54,
    info: '54321 → 80 [ACK] Seq=1 Ack=1 Win=64240',
    hex: '4500 0036 d4f3 4000 4006 c385 c0a8 0169 5d80 d822 d431',
    ascii: 'E..6..@.@......i]..".1..ACK...',
    details: { layer: 'Transmission Control Protocol (ACK)', headers: ['Flags: [ACK]', 'Seq Number: 1', 'Ack Number: 1'] }
  },
  {
    no: 6, time: '0.052', src: '192.168.1.105', dst: '93.184.216.34', proto: 'HTTP', len: 480,
    ...
    details: { layer: 'Hypertext Transfer Protocol (GET Request)', headers: ['Method: GET', 'URI: /login.php', 'Host: example.com', 'User-Agent: Mozilla/5.0 (Kali Linux)'] }
  }
];
...
export function PacketCaptureViz() {
  ...
  const [newPacketFlash, setNewPacketFlash] = useState<number | null>(null);

  useEffect(() => {
    if (!isCapturing) return;
    const interval = setInterval(() => {
      setStreamIndex((prev) => {
        const next = prev >= PACKETS.length ? 1 : prev + 1;
        setNewPacketFlash(next);
        setTimeout(() => setNewPacketFlash(null), 600);
        return next;
      });
    }, 1500);
    ...
    <div className="space-y-5 font-mono select-none w-full">
      <div className="flex items-center justify-between gap-2 p-3 rounded-xl bg-[#F7F7F7] dark:bg-[#141414] border border-[#E5E5E5] dark:border-[#2A2A2A]">
        ...
        <button
          ...
          className="p-2 rounded-lg bg-white dark:bg-[#1C1C1C] border border-[#E5E5E5] dark:border-[#2A2A2A] text-[#111111] dark:text-white hover:border-[#111111] dark:hover:border-white transition-colors duration-300 flex items-center gap-1.5 text-xs font-bold uppercase shrink-0 cursor-pointer"
        >
          {isCapturing ? <Pause size={14} /> : <Play size={14} />}
          <span className="hidden sm:inline">{isCapturing ? 'PAUSE' : 'START'}</span>
        </button>
      </div>
        ...
        <div className="divide-y divide-[#E5E5E5] dark:divide-[#2A2A2A] max-h-[220px] overflow-y-auto">
          <AnimatePresence>
            {filteredPackets.map((p) => {
              ...
              const isNew = p.no === newPacketFlash;
              return (
                <motion.button
                  key={p.no}
                  id={`pcap-row-${p.no}`}
                  type="button"
                  onClick={() => { setSelectedNo(p.no); setIsCapturing(false); }}
                  initial={isNew ? { opacity: 0, x: -10 } : false}
                  animate={{ opacity: 1, x: 0 }}
                  transition={trans}
                  className={`w-full grid grid-cols-12 gap-2 p-3 text-left text-xs font-mono transition-colors duration-300 cursor-pointer ${
                    isSelected
                ...
                >
                  ...
                  <span className={`col-span-1 font-extrabold ${p.proto === 'HTTP' ? 'text-[#555555]' : p.proto === 'TCP' ? 'text-[#333333]' : 'text-[#777777]'}`}>
                    {p.proto}
                  </span>
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            ...
            <div className="p-3.5 rounded-lg bg-[#F7F7F7] dark:bg-[#181818] border border-[#E5E5E5] dark:border-[#2A2A2A] space-y-2 font-mono">
              ...
              <code className="block text-xs text-[#333333] dark:text-[#B5B5B5] break-all leading-relaxed font-bold">
