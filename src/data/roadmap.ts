...
**Integrity** means that a transaction, once recorded, cannot be silently altered. If you transfer £200 to a friend, the record of that transfer must be immutable. Banks enforce this with cryptographic checksums, database write ahead logs, and audit trails. If an attacker could modify a transaction in transit  say, changing the recipient account number  integrity would be violated. This is exactly what a man in the middle attack attempts.

**Availability** means the banking app must be up and responsive when you need it. Banks invest heavily in redundant servers, DDoS protection, and failover systems so that a denial of service attack (or just a traffic spike on payday) does not take the service offline.

**Career paths at a glance:**
- **SOC Analyst**: You sit in a Security Operations Centre, monitoring dashboards and alerts 24/7. When an alert fires, you triage it  is this a real threat or a false positive? Typical work includes reviewing SIEM logs, escalating incidents, and writing incident reports.
- **Penetration Tester**: You are paid to try to break into systems  legally, with written permission. You find vulnerabilities before the bad actors do. Typical work includes scoping engagements, running tools like Nmap or Burp Suite, and writing professional reports.
- **AppSec Engineer**: You work alongside developers to build security into software from the start. Typical work includes threat modeling new features, reviewing pull requests for security flaws, and configuring SAST and DAST tools in CI pipelines.
- **GRC Analyst**: You translate legal and regulatory requirements (GDPR, ISO 27001, SOC 2) into internal policies and ensure the organisation follows them. Typical work includes risk assessments, policy writing, and audit preparation.`,
        },
        resources: [
          {
          ...
          },
          {
            ...
            description: 'A respected certification body\'s overview of how roles interconnect and what skills each requires.',
          },
          {
      ...
      },
      {
        ...
        learn: [
          ...
          'What a Virtual Machine (VM) is: a software based computer running inside your real computer, fully isolated from your main OS.',
          'Why VirtualBox + Kali Linux is an industry standard beginner lab setup  free, widely tested, and well documented.',
        ...
        doLab: [
          ...
          'Download the official Kali Linux VirtualBox image (the ready to use .ova file) from kali.org/get-kali  this saves hours compared with a manual installation.',
...
**Processors:** 2 CPUs. Kali uses multiple threads; giving it 2 virtual CPUs makes tools like Nmap's parallel scanning noticeably faster.
...
**Network Adapter (for now):** NAT. This lets the VM access the internet through your host machine for updates, while keeping it invisible to your local network. In Day 8 you will switch to a host only network for isolated lab practice.
...
sudo apt update          # Fetches package lists  if this works, the lab is alive`,
        ...
        resources: [
          {
            ...
            description: 'The official VirtualBox download page  always use this; avoid external mirrors.',
          },
          {
      ...
      },
      {
        ...
        learn: [
          ...
          'IPv4 versus IPv6: why IPv4 (32 bit, about 4.3 billion addresses) is running out and IPv6 (128 bit) is the long term answer.',
...
#             If TTL reaches 0, the packet is dropped (prevents loops).`,
      ...
      },
      {
        ...
        learn: [
          ...
          'How data is encapsulated as it travels down the stack (headers are added at each layer) and unpacked on the receiving end.',
          ...
          'The difference between TCP (reliable, ordered, connection based) and UDP (fast, connectionless, no delivery guarantee).',
        ],
        doLab: [
...
Network: 192.168.1.192  Broadcast: 192.168.1.255  Hosts: 62`,
      ...
      },
      {
        ...
        learn: [
          ...
          'Port categories: System (0–1023, reserved for system services), Registered (1024–49151), Dynamic or Ephemeral (49152–65535, used by clients for outgoing connections).',
          'Essential common ports: 20/21 FTP, 22 SSH, 25 SMTP, 53 DNS, 80 HTTP, 443 HTTPS, 3306 MySQL, 3389 RDP.',
          'Protocol details: HTTP (stateless, plaintext), HTTPS (HTTP over TLS), SSH (encrypted remote shell), FTP (file transfer, avoid in favour of SFTP), SMTP (email sending).',
          'The difference between TCP ports (connection based, reliable) and UDP ports (connectionless  used by DNS, NTP, video streaming).',
        ],
        doLab: [
...
# 127.0.0.1 addresses are local  much safer for database services.`,
        },
        resources: [
          {
            title: 'IANA  Service Name and Transport Protocol Port Number Registry',
            url: 'https://www.iana.org/assignments/service-names-port-numbers/service-names-port-numbers.xhtml',
            description: 'The authoritative registry of every common and registered port number. Bookmark this.',
          },
          {
      ...
      },
      {
        ...
        learn: [
          'What packet capturing (also called "packet sniffing") is: intercepting and recording network traffic as raw bytes, then decoding it into readable protocol layers.',
        ...
        doLab: [
          'Download and install Wireshark from wireshark.org. On Linux/Kali it is already installed: run `sudo wireshark`.',
          'Select your active network interface (for example eth0 or Wi-Fi) and click the blue shark fin Start button.',
...
# On HTTPS (port 443) you would see TLS Application Data instead.`,
      ...
      },
      {
        ...
        learn: [
          ...
          'Host only networking: the VM gets a private IP on a virtual network shared only between VMs and the host. No internet  perfect for isolated attack and defence practice.',
        ...
        doLab: [
          'In VirtualBox, go to File → Host Network Manager (or Tools → Network in newer versions). Create a new host only adapter if one does not exist. Note its IP range (for example 192.168.56.0/24).',
          'Open your Kali Linux VM settings → Network → Adapter 2. Enable it, set to the Host Only Adapter option, then select the adapter you just created.',
          ...
          'Configure the second VM\'s network adapter to use host only networking as well.',
        ...
        example: {
          ...
          prose: `Here is the exact sequence for setting up an isolated two virtual machine lab in VirtualBox:
...
**Step 2  Assign a Host Only Adapter to Kali VM:**
In Kali VM Settings → Network → Adapter 1: keep as NAT (for internet access). Adapter 2: enable, set to the Host Only Adapter option, and select vboxnet0.

**Step 3  Assign a Host Only Adapter to Target VM (Metasploitable 2):**
In Target VM Settings → Network → Adapter 1: set to the Host Only Adapter option and select vboxnet0. No NAT is needed  this machine should not have internet access.
...
# Your isolated lab is now operational.`,
        ...
        resources: [
          {
            ...
            description: 'The definitive reference on all VirtualBox networking modes  NAT, host only, bridged, and internal  with diagrams.',
          },
          {
      ...
      },
      {
        ...
        doLab: [
          ...
          'Check the review list below  if any item gives you pause, revisit that day\'s material before proceeding to Module 2.',
        ],
        example: {
          title: 'Review Checklist  One Key Command Per Day',
